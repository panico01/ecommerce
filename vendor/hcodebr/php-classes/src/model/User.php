...
Class User extends Model {
    ...
    public static function getForgot($email)
    {
        ...
        $results = $sql->select("SELECT * FROM tb_persons a INNER JOIN tb_users b USING(idperson)", array(
            ":email"=>$email
        ));

        if (count($results) === 0){

            throw new \Excpetion("Não foi possivel recuperar a senha.");
    ...
    public static function validForgotDecrypt($code){
        ...
        if (count($results) === 0){
            throw new \Exception("Não foi possível recuperar a senha", 1); //dt recovery já foi setado ou o tempo foi expirado
